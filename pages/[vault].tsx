...
function VaultItem({asset}: VaultItemProps) {
    return (
        ...
        <div className="desc p-4 text-gray-800">
          <span className="description text-sm block py-2 border-gray-400 mb-2">{asset.name}</span>
        </div>
      </div>
    )
}
...
function VaultCollection({vault}: VaultsProps) {
    ...
    const [url, setUrl] = useState(`https://api.opensea.io/api/v1/assets?asset_contract_address=${vaultsJson[vault].address}&token_ids=${vaultsJson[vault].ids.slice(offset,limit).join('&token_ids=')}&offset=${offset}&limit=${limit}`)
    ...
    return (
        <>
            ...
            {/* see more button */}
            {vaultsJson[vault].ids < collection.length && <button onClick={seeMore}>more</button>}
        </>
    )
}
