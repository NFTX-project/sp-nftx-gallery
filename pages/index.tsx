    ...
    <section>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
        {vaults.map((vault, i) => (
          ...
          >
            <a
              className={`flex p-6 font-mono break-all bg-${colorMap[vault.name]}`}
            >
              <div className="flex-none h-20 w-20 sm:w-28 sm:h-28 relative">
                <img src={`https://via.placeholder.com/160x160.png?text=${vault.name}%20image`} alt="" className="absolute inset-0 w-full h-full object-cover border border-black shadow-offset-lime" />
              </div>
